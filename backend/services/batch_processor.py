...
class BatchProcessor:
        ...
        try:
            ...
            for idx, old_decision in enumerate(decisions_to_process, 1):
                try:
                    ...
                    new_result = self.compliance_engine.evaluate_transaction(transaction)
                    
                    successfully_evaluated += 1
                    
                    # Compare verdicts
                    old_verdict = old_decision.get("verdict")
                    ...
                    if old_verdict != new_verdict:
                        ...
                        changes.append({
                            ...
                            "decision_id": old_decision.get("decision_id"),
                            "old_verdict": old_verdict,
                            "new_verdict": new_verdict,
                            "old_risk_score": old_decision.get("risk_score"),
                            "new_risk_score": new_result.get("risk_score"),
                            "re_evaluation_date": datetime.now().isoformat(),
                            "reason_for_change": self._analyze_change_reason(old_decision, new_result)
                        })
    ...
    def reevaluate_by_policy(self, policy_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            filter_by = {"decision_ids": [d["decision_id"] for d in affected_decisions]}
            return self.reevaluate_all_decisions(filter_by)
        ...
        try:
            ...
            for decision in all_decisions:
                ...
                if timestamp < cutoff_date:
                    ...
                    if verdict_filter is None or verdict == verdict_filter:
                        candidates.append({
    ...
    def _apply_filters(
        ...
        if "decision_ids" in filters:
            decision_ids = set(filters["decision_ids"])
            filtered = [d for d in filtered if d.get("decision_id") in decision_ids]
    ...
    def _analyze_change_reason(
        ...
        # Compare risk scores
        old_risk = old_decision.get("risk_score", 0)
        new_risk = new_result.get("risk_score", 0)
        ...
        old_citations = len(old_decision.get("citations", []))
        new_citations = len(new_result.get("citations", []))
        
        if old_citations != new_citations:
            reasons.append(f"Policy citations changed from {old_citations} to {new_citations}")
        
        # Compare reasoning
        old_reasoning = old_decision.get("reasoning", "")
